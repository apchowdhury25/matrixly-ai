"""JSON session memory for booking chat."""
...
class SessionStore:
    ...
    def append_messages(self, session_id: str, messages: list[ChatMessage]) -> dict[str, Any]:
        doc = self.get(session_id) or self.create()
    ...
    def update_context(self, session_id: str, **ctx: Any) -> dict[str, Any]:
    ...
    def history(self, session_id: str, limit: int = 20) -> list[ChatMessage]:
        ...
        if not doc:
            return []
        return [ChatMessage(**m) for m in (doc.get("messages") or [])[-limit:]]
