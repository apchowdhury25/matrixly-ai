...
def load_config() -> dict[str, Any]:
    ...
    raw["security"] = {
        "api_key": os.getenv("DOCFORGE_API_KEY", "change-me-admin-key"),
        "widget_key": os.getenv("DOCFORGE_WIDGET_KEY", "pk_live_change-me"),
    }
    ...
    raw["hubspot"] = {"access_token": os.getenv("HUBSPOT_ACCESS_TOKEN", "")}
    raw["salesforce"] = {
...
def brand_text(cfg: dict) -> str:
    brand = cfg.get("brand") or {}
    rel = brand.get("voice_file") or "brand/guidelines.md"
    path = ROOT / rel
    if path.exists():
        return path.read_text(encoding="utf-8")
    return "Professional B2B document voice."
