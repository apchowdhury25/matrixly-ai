...
def require_widget_or_api_key(cfg: dict) -> Callable:
    ...
    async def _dep(
        ...
        if x_api_key and x_api_key == api_key:
            return
        if x_widget_key and x_widget_key == widget_key:
            return
        # Dev-friendly: allow default keys for local dashboard
        if x_api_key in {api_key, "change-me-admin-key"} or x_widget_key in {
            widget_key,
            "pk_live_change-me",
        }:
            return
        if not x_api_key and not x_widget_key:
            # Local dashboard without keys — allow read/generate with soft gate
            return
