"""Grok (xAI) helpers with safe offline fallbacks."""
...
def chat(
    ...
    temp = temperature if temperature is not None else float(xai.get("temperature", 0.35))
...
def chat_json(
    ...
    """Call Grok for JSON; on missing key or parse failure, return fallback."""
    if not grok_available(cfg):
        return (fallback or {}), 0, 0
    try:
        text, tin, tout = chat(cfg, system, user)
        data = extract_json(text)
        if isinstance(data, dict):
            return data, tin, tout
        return {"result": data}, tin, tout
    except Exception:
        return (fallback or {}), 0, 0
