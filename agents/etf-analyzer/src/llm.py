...
def cost_usd(cfg: dict, tokens_in: int, tokens_out: int) -> float:
    ...
    return (tokens_in / 1_000_000.0) * float(cost.get("input_per_1m_usd", 5.0)) + (
        tokens_out / 1_000_000.0
    ) * float(cost.get("output_per_1m_usd", 15.0))


def chat(cfg: dict, system: str, user: str) -> tuple[str, int, int]:
    xai = cfg.get("xai") or {}
    key = xai.get("api_key")
    if not key:
    ...
    temp = float(xai.get("temperature", 0.2))
