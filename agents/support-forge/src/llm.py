"""Grok (xAI) helpers — same pattern as other Matrixly agents."""
...
def chat(
    ...
    Returns (content, tokens_in_est, tokens_out_est).
    """
    xai = cfg.get("xai") or {}
    key = xai.get("api_key")
    if not key:
        raise RuntimeError("XAI_API_KEY not set — use rule-based mode")
