...
class TestPublicPagesPlaywright:
    def test_each_public_page_has_title(self, page, site_base_url):
        for name in PUBLIC_PAGES:
            url = f"{site_base_url}/{name}" if name else f"{site_base_url}/"
            page.goto(url, wait_until="domcontentloaded")
            title = page.title()
            assert title and len(title) > 2, f"{name or 'home'} missing title"

    def test_agents_grid_cards(self, page, site_base_url):
        page.goto(f"{site_base_url}/agents", wait_until="domcontentloaded")
    ...
    def test_admin_gate(self, page, site_base_url):
        page.goto(f"{site_base_url}/admin", wait_until="domcontentloaded")
